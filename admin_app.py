...
@media (max-width: 700px) {
    ...
    /* Streamlit columns stack naturally */
    [data-testid="column"] {
        min-width: 0 !important;
    }
...
if not st.session_state.user:
    ...
    with tab_list:
        ...
        villages = ["सभी"] + sorted(
            set(c["village"] for c in all_complaints)
        ) if all_complaints else ["सभी"]
        ...
        search_query = st.text_input(
            "🔎 शिकायत खोजें",
            placeholder="Complaint ID, नाम, मोबाइल, गाँव, category या शिकायत लिखें...",
            key="complaint_search"
        ).strip().lower()

        f1, f2, f3 = st.columns(3)

        status_options = ["सभी"] + db.STATUS_FLOW
        village_options = villages
        urgency_options = ["सभी", "High", "Medium", "Low"]

        # Safe defaults — only initialize before widgets are created
        if "complaint_status_filter" not in st.session_state:
            st.session_state["complaint_status_filter"] = "सभी"

        if "complaint_village_filter" not in st.session_state:
            st.session_state["complaint_village_filter"] = "सभी"

        if "complaint_urgency_filter" not in st.session_state:
            st.session_state["complaint_urgency_filter"] = "सभी"
        ...
        village_f = f2.selectbox(
            "गाँव",
            village_options,
            key="complaint_village_filter"
        )

        urgency_f = f3.selectbox(
            "प्राथमिकता",
            urgency_options,
            key="complaint_urgency_filter"
        )

        def clear_complaint_filters():
            st.session_state["clear_complaint_filters_trigger"] = True

        clear_clicked = st.button(
            "✖️ Clear Filters",
            key="clear_complaint_filters"
        )

        if clear_clicked:
            st.session_state.pop("complaint_search", None)
            st.session_state.pop("complaint_status_filter", None)
            st.session_state.pop("complaint_village_filter", None)
            st.session_state.pop("complaint_urgency_filter", None)
            st.rerun()
        ...
        # Search across important complaint fields
        if search_query:
            def complaint_matches(c):
                searchable_fields = [
                    c.get("id"),
                    c.get("citizen_name"),
                    c.get("citizen_phone"),
                ...
                return any(
                    search_query in str(value or "").lower()
                    for value in searchable_fields
                )

            complaints = [
                c for c in complaints
                if complaint_matches(c)
            ]

        st.caption(
            f"📋 {len(complaints)} शिकायतें दिखाई जा रही हैं"
        )
        ...
        for c in complaints:
            selected = st.checkbox(
                f"Select {c['id']}",
                key=f"select_complaint_{c['id']}"
            )

            if selected:
                selected_complaints.append(c["id"])

        if selected_complaints:
            st.info(
                f"☑️ {len(selected_complaints)} शिकायतें selected हैं"
            )

            bulk_status = st.selectbox(
                "📌 Selected complaints की नई स्थिति",
                db.STATUS_FLOW,
                key="bulk_status"
            )
            ...
            if st.button(
                ...
                for complaint_id in selected_complaints:
                    ...
                    if complaint:
                        db.update_status(
                            complaint_id,
                            bulk_status,
                            notes=complaint.get("resolution_notes"),
                            assigned_officer=(
                                complaint.get("assigned_officer")
                                or complaint.get("assigned_to")
                                or user["username"]
                            ),
                            resolution_remarks=complaint.get(
                                "resolution_remarks"
                            ),
                            resolution_photo=complaint.get(
                                "resolution_photo"
                            ),
                ...
                st.success(
                    f"✅ {updated_count} शिकायतों की स्थिति अपडेट हो गई।"
                )
                st.rerun()
        ...
        for c in complaints:
            ...
            with st.expander("विवरण / Update status"):
                ...
                # =================================================
                # OPTIONAL — ADMIN DELETE RESOLVED/CLOSED COMPLAINT
                # =================================================
                if user["role"] == "admin":

                    with st.expander("🗑️ Complaint Management"):
                        if c["status"] in ("Resolved", "Closed"):

                            st.warning(
                                "⚠️ यह complaint permanently delete होगी। "
                                "यह action केवल Resolved या Closed complaint के लिए उपलब्ध है।"
                            )

                            confirm_delete = st.checkbox(
                                "मैं इस complaint को permanently delete करना चाहता हूँ।",
                                key=f"confirm_delete_{c['id']}"
                            )
                            ...
                            if st.button(
                                ...
                                deleted, delete_msg = db.delete_complaint(c["id"])
                            ...
                            st.info(
                                "🔒 Delete option केवल Resolved या Closed complaint के लिए उपलब्ध है।"
                            )
